...
# Left hand {{{4
cg.setConstraints (node = 'left',
                   constraints = Constraints (grasps = ['l_grasp',]))

cg.createWaypointEdge ('free', 'left', 'l_grasp', 1, 10, 'free', True)

cg.setConstraints (edge='l_grasp_e1',
                   constraints = Constraints (lockedJoints = lockbox))
cg.setConstraints (node='l_grasp_n0',
                   constraints = Constraints (pregrasps = ['l_pregrasp',]))
cg.setConstraints (edge='l_grasp_e0',
                   constraints = Constraints (lockedJoints = lockbox))
# 4}}}

# Both hands {{{4
cg.setConstraints (node='both',
                   constraints = Constraints (grasps = ['l_grasp', 'r_grasp']))

cg.createWaypointEdge ('both', 'right', 'b_l_ungrasp', 1, 1, 'right', True)

cg.createWaypointEdge ('right', 'both', 'b_l_grasp', 1, 10, 'right', True)

cg.setConstraints (node='b_l_ungrasp_n0',
                   constraints = Constraints (pregrasps = ['l_pregrasp',]))
cg.setConstraints (edge='b_l_ungrasp_e0',
                   constraints = Constraints (lockedJoints = lockbox))
cg.setConstraints (edge='b_l_grasp_e1',
                   constraints = Constraints (lockedJoints = lockbox))
cg.setConstraints (node='b_l_grasp_n0',
                   constraints = Constraints (pregrasps = ['l_pregrasp',]))

cg.createWaypointEdge ('both', 'left', 'b_r_ungrasp', 1, 1, 'left', True)

cg.createWaypointEdge ('left', 'both', 'b_r_grasp', 1, 10, 'left', True)

cg.setConstraints (node='b_r_ungrasp_n0',
                   constraints = Constraints (pregrasps = ['r_pregrasp',]))
cg.setConstraints (edge='b_r_ungrasp_e0',
                   constraints = Constraints (lockedJoints = lockbox))
cg.setConstraints (edge='b_r_grasp_e1',
                   constraints = Constraints (lockedJoints = lockbox))
cg.setConstraints (node='b_r_grasp_n0',
                   constraints = Constraints (pregrasps = ['r_pregrasp',]))

# 4}}}

# Loops {{{4
cg.createEdge ('free', 'free', 'move_free', 1, 'free')

cg.createEdge ('left', 'left', 'l_keep_grasp', 5, 'left')

cg.createEdge ('right', 'right', 'r_keep_grasp', 5, 'right')

cg.createEdge ('both', 'both', 'move_both', 1, 'both')

cg.setConstraints (edge='move_free',
                   constraints = Constraints (lockedJoints = lockbox))
# 4}}}
